...
{
	/*
	[Attribute("US", "auto", "The side designated as blufor, this faction will have control of the beginning half of the total zones at game start. \n\n In Example: If the total zones were [A, B, C, D, E] this faction would have control of [A, B] at game start", category: "GunGame Faction Settings")]
	FactionKey m_BluforSide;
	
	[Attribute("USA", "auto", "Nickname for the side designated as blufor", category: "GunGame Faction Settings")]
	string m_sBluforSideNickname;
	
	[Attribute("USSR", "auto", "The side designated as opfor, this faction will have control of the last half of the total zones at game start. \n\n In Example: If the total zones were [A, B, C, D, E] this faction would have control of [D, E] at game start", category: "GunGame Faction Settings")]
	FactionKey m_OpforSide;
	
	[Attribute("USSR", "auto", "Nickname for the side designated as opfor", category: "GunGame Faction Settings")]
	string m_sOpforSideNickname;
	
	*/
	[Attribute("", UIWidgets.EditBox, desc: "Array of all zone object names, !MAKE SURE ALL OBJECTS LISTED ARE INVINCIBLE!", category: "GunGame Zone Settings")]
	ref array<string> m_aZoneObjectNames;
	
	/*
	[Attribute("30", "auto", "[Seconds] The amount of time it takes to cap a zone", category: "GunGame Zone Settings")]
	int m_iZoneCaptureTime;
	
	[Attribute("600", "auto", "[Seconds] Time until the GunGame zones are unlocked after being locked, recommend you stick to on of these: [5 minutes, 10 minutes, 15 minutes, 20 minutes]", category: "GunGame Zone Settings")]
	int m_iZoneUnlockTime;
	
	[Attribute("5", "auto", "Min number of players needed to cap a zone", category: "GunGame Zone Settings")]
	int m_iMinNumberOfPlayersNeeded;
	
	[Attribute("600", "auto", "[Seconds] When all zones are captured by a side, it'll take this set time to declare that side a victor", category: "GunGame Zone Settings")]
	int m_iTimeToWin;
	
	[Attribute("900", "auto", "[Seconds] Time until the middle zone is unlocked", category: "GunGame Zone Settings")]
	int m_iInitialTime;
	...
	static CRF_GunGameGameModeComponent GetInstance()
	{
		BaseGameMode gameMode = GetGame().GetGameMode();
		if (gameMode)
			return CRF_GunGameGameModeComponent.Cast(gameMode.FindComponent(CRF_GunGameGameModeComponent));
		else
			return null;
	}
